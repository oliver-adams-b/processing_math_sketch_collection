...
float r1 = 150; // length of first pendulum
float r2 = 80; // length of second pendulum
float m1 = 10; //  mass of first pendulum excluding weight of string
float m2 = 30; // mass of second pendulum excluding weight of string
...
void draw() {
  ...
  pushStyle(); 
  noFill(); 
  ellipse(0, 0, 2*(r1 + r2), 2*(r1 + r2)); 
  
  for(int i = 1; i < points.size(); i++){
   PVector p1 = points.get(i-1); 
   PVector p2 = points.get(i); 
   
   PVector transform_p1 = mobiusTransform(new PVector(0, 0), 
                                          p1, 2*(r1 + r2)); 
   PVector transform_p2 = mobiusTransform(new PVector(0, 0), 
                                          p2, 2*(r1 + r2)); 
   stroke(255, 153, 153);                                 
